...
const { INTERNAL_DIR, safeName, isInside } = require('./paths');
 ...
 */
function add(config, { label, targetPath, library = null }) {
  ...
  // A "drive" that is really a folder inside the library — or the library
  // itself, or anything containing it — is a loop waiting to happen. Sync a
  // folder to somewhere inside itself and every run copies the previous run's
  // copy: the library grows without bound until the disk is full, unattended,
  // because sync-on-connect will keep doing it. Relocating an album into the
  // library it came from is equally meaningless.
  if (library) {
    const lib = path.resolve(library);
    if (isInside(lib, resolved)) {
      throw new LocationError(
        'That folder is inside your library. Choose somewhere outside it — another '
        + 'drive, or a folder your cloud service syncs.',
      );
    }
    if (isInside(resolved, lib)) {
      throw new LocationError(
        'That folder contains your library, so copying into it would copy the library '
        + 'into itself. Choose a folder outside it.',
      );
    }
  }
...
/** Where deletions go on a drive that holds relocated albums. */
const DRIVE_TRASH = '.lanshare-trash';

/**
 * Delete a relocated album.
 *
 * Trashing only the link — which is what an ordinary delete does, since the
 * link is what sits in the library — leaves the album's real contents on the
 * other drive with nothing pointing at them: gigabytes that no longer appear
 * anywhere in the app and that nothing will ever clean up.
 *
 * So the real folder is moved into a trash folder **on its own drive**, for
 * the same reason a vault file is trashed inside its vault: a deletion should
 * stay with the data it belongs to, and stay recoverable.
 *
 * Refuses when the drive is not connected. There is no honest way to delete
 * something you cannot reach, and pretending otherwise is how the orphaning
 * happens in the first place.
 */
async function trashAlbum(library, config, name, stamp = String(Date.now())) {
  const linkPath = path.join(library, name);
  if (!isLink(linkPath)) throw new LocationError('That album is not on another drive');

  const target = linkTarget(linkPath);
  if (!target || !fs.existsSync(target)) {
    throw new LocationError(
      'That album lives on a drive that is not connected. Connect it to delete the album, '
      + 'or bring the album back first.',
      409,
    );
  }

  // The drive's own root, so the trash sits beside the albums rather than
  // inside one of them.
  const owner = list(config).find((loc) => loc.path
    && path.resolve(target).startsWith(path.resolve(loc.path)));
  const trashRoot = owner?.path ? path.join(owner.path, DRIVE_TRASH) : path.join(path.dirname(target), DRIVE_TRASH);

  const destination = path.join(trashRoot, stamp);
  await fsp.mkdir(destination, { recursive: true });

  // The contents move first. If this fails the link is still there and the
  // album still works, which is the state worth failing into.
  const moved = path.join(destination, path.basename(target));
  try {
    await fsp.rename(target, moved);
  } catch (err) {
    if (err.code !== 'EXDEV') throw err;
    await copyTree(target, moved);
    await fsp.rm(target, { recursive: true, force: true });
  }

  removeLink(linkPath);
  return { name, trashedTo: moved };
}
