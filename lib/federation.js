...
const { IndexDb, dbRowToResult, resultToUpsertEntry, haversineKm } = require('./index-db');
...
/**
 * Is this actually a search result, or just something a peer sent?
 *
 * A peer's response is untrusted input like any other. It arrives over an
 * authenticated channel, which proves it came from the machine that was
 * paired with — not that the machine is healthy, running the same version,
 * or still under its owner's control. Before this check, a row whose `path`
 * was null (or a number, or missing) threw a TypeError inside the caching
 * loop below, and because federatedSearch fans out with Promise.all that
 * one row from one peer rejected the entire search — taking the asking
 * machine's *own* local results down with it. Exactly the outcome
 * taggedFromCache() exists to prevent for an unreachable peer, arrived at
 * by a different route.
 *
 * Only the fields that are load-bearing are insisted on: `path` and `name`
 * because they are used to build strings and index rows, `kind` and the two
 * numbers because lib/index-db.js declares them NOT NULL. Everything else
 * is allowed to be missing or null, which is ordinary — a file with no EXIF
 * has no camera and no capture date.
 */
function isUsablePeerRow(row) {
  ...
  if (!Number.isFinite(row.size) || !Number.isFinite(row.mtime)) return false;
  return true;
}
...
async function searchOnePeer({ library, config, conn, secrets, params, timeoutMs }) {
  ...
  try {
    ...
    // Rows that are not usable are dropped rather than refused wholesale: a
    // peer part-way through an upgrade, or with one damaged index row, still
    // has plenty worth showing, and one bad row is not a reason to lose the
    // rest of that machine's answer.
    const results = raw.filter(isUsablePeerRow);
    ...
    return {
      ...
      report: {
        ...
        label: conn.label,
        reachable: true,
        ...(dropped ? { note: `${dropped} result${dropped === 1 ? '' : 's'} from this machine could not be read` } : {}),
      },
    };
  } finally {
    cache.close();
...
/**
 * searchOnePeer already turns every *expected* problem into a cached-or-
 * empty answer, so reaching this is by definition a surprise. It still must
 * not be fatal: federatedSearch fans out across peers, and one machine
 * behaving in a way nobody anticipated should cost its own results, not the
 * whole search and not the local ones.
 */
async function searchOnePeerSafely(args) {
  try {
    return await searchOnePeer(args);
  } catch (err) {
    return {
      results: [],
      report: {
        id: args.conn.id, label: args.conn.label, reachable: false, error: err.message,
      },
    };
  }
}
...
/**
 * Whether one published entry belongs in these results, and — for a GPS
 * search — the distance that earns it its place in the sort.
 *
 * A central-index entry never touched SQLite, so it is filtered here in JS
 * against the exact same params the local SQL query used, mirroring what
 * lib/index-db.js's search() does over its own rows. It is deliberately not
 * identical: text matching here is a plain case-insensitive substring check
 * on name and path, not FTS5's tokenized prefix match. That is a real,
 * accepted difference — simpler, and correct for the common case of typing
 * a word or a piece of a filename — not an oversight.
 */
function centralEntryIfMatches(entry, params) {
  if (params.kind && entry.kind !== params.kind) return null;
  if (params.cameraMake && entry.cameraMake !== params.cameraMake) return null;
  if (params.from && (!entry.capturedAt || entry.capturedAt < params.from)) return null;
  if (params.to && (!entry.capturedAt || entry.capturedAt > params.to)) return null;
  if (params.text) {
  ...
  if (params.near) {
    if (!Number.isFinite(entry.gpsLat) || !Number.isFinite(entry.gpsLon)) return null;
    const distanceKm = haversineKm(params.near.lat, params.near.lon, entry.gpsLat, entry.gpsLon);
    if (distanceKm > (params.radiusKm || 5)) return null;
...
/**
 * Fetch the shared central index (Phase J) — the roster, then every device
 * listed in it, in parallel — and return whatever entries match this
 * search. `centralKeys` is `{encryptionKey, lookupKey}`, already derived
 * from the shared passphrase by the caller once and reused across requests;
 * deriving them is a deliberately slow PBKDF2 call, not something to redo
 * on every search.
 *
 * One corrupt or undecryptable device blob (a stale entry left in the
 * roster for a machine that changed its passphrase, say) is skipped rather
 * than failing the whole fetch — the other devices' entries are still worth
 * having.
 */
async function fetchCentralIndex({ config, centralKeys, params, timeoutMs }) {
  const { relayHost, relayPort } = config.centralIndex || {};
  if (!relayHost || !relayPort || !centralKeys) return { rows: [], report: null };

  const relay = { relayHost, relayPort, timeoutMs };
  try {
    const rosterKey = centralIndexLib.rosterSlotKey(centralKeys.lookupKey);
    const rosterMsg = await withTimeout(
      () => tunnel.storeRequest({ ...relay, action: 'get', key: rosterKey }),
      timeoutMs, 'The central index did not answer in time',
    );
    if (!rosterMsg.blob) return { rows: [], report: { reachable: true, devices: 0 } };

    const roster = centralIndexLib.open(
      centralKeys.encryptionKey, centralIndexLib.ROSTER_AAD, Buffer.from(rosterMsg.blob, 'base64'),
    );

    const perDevice = await Promise.all((roster.devices || []).map(async (device) => {
      try {
        const slotKey = centralIndexLib.deviceSlotKey(centralKeys.lookupKey, device.deviceId);
        const msg = await withTimeout(
          () => tunnel.storeRequest({ ...relay, action: 'get', key: slotKey }),
          timeoutMs, `${device.label || device.deviceId} did not answer in time`,
        );
        if (!msg.blob) return [];
        const deviceBlob = centralIndexLib.open(
          centralKeys.encryptionKey, centralIndexLib.deviceAad(device.deviceId), Buffer.from(msg.blob, 'base64'),
        );
        return (deviceBlob.entries || [])
          .map((entry) => centralEntryIfMatches(entry, params))
          .filter(Boolean)
          .map((entry) => withLocation(entry, {
            source: `central:${device.deviceId}`, label: device.label || deviceBlob.label,
            path: entry.path, reachable: false, cachedAt: deviceBlob.publishedAt,
          }));
      } catch {
        return [];
      }
    ...
    return { rows: perDevice.flat(), report: { reachable: true, devices: roster.devices?.length || 0 } };
  } catch (err) {
    return { rows: [], report: { reachable: false, error: err.message } };
  }
}

/**
 * Search every paired machine and the shared central index, and merge with
 * what was already found locally. Always runs the merge, even with nothing
 * else configured — the same pass is also what collapses two local copies
 * of one file (uploaded twice, say) into a single result naming both paths,
 * which is worth having regardless of federation.
 */
async function federatedSearch({
  library, config, secrets, localResults, params, centralKeys = null, timeoutMs = DEFAULT_TIMEOUT_MS,
}) {
  const localTagged = localResults.map((r) => withLocation(r, localLocation(r)));
  const connectionList = config.connections || [];

  const [peerOutcomes, central] = await Promise.all([
    Promise.all(connectionList.map((conn) => searchOnePeerSafely({
      library, config, conn, secrets, params, timeoutMs,
    }))),
    fetchCentralIndex({ config, centralKeys, params, timeoutMs }),
  ]);
  ...
  const peerTagged = peerOutcomes.flatMap((outcome) => outcome.results);

  return {
    results: mergeByHash([...localTagged, ...peerTagged, ...central.rows], { near: params.near }),
    peers: peerOutcomes.map((outcome) => outcome.report),
    centralIndex: central.report,
  };
}
