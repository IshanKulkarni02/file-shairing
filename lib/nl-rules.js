...
<destination> is an absolute path starting with "/", optionally containing:
  {year} {month} {day}   from the file's own capture date
  {camera}               the camera's model, or its make when there is no model
  {make} {model}         the two parts separately

Never invent a place's coordinates; write the place name in quotes and let "gps near" \
resolve it.
...
/**
 * Which models this Ollama actually has, newest-installed first.
 *
 * Exists because the failure it prevents was genuinely baffling to hit:
 * asking for a model that is not installed returns **404**, which reads
 * exactly like "server not running" but happens only when the server *is*
 * running. Without the list there is nothing to say beyond the status code,
 * and the obvious next move — restart Ollama — cannot possibly help.
 *
 * Returns [] rather than throwing: this is only ever used to make an error
 * message more useful, and an error about fetching the list would bury the
 * error the person actually came here for.
 */
async function listModels({ host = DEFAULT_HOST, fetchImpl = (...args) => fetch(...args) } = {}) {
  try {
...
async function callModel({
  ...
  // 404 from a *running* Ollama means the model is not installed, which is
  // the one failure here with an exact fix. Saying only "refused (404)" sent
  // at least one person restarting a server that was working fine.
  if (res.status === 404) {
    const installed = await listModels({ host, fetchImpl });
    throw new NlRulesError(
      installed.length
        ? `"${model}" is not installed in Ollama. Either run "ollama pull ${model}", `
          + `or pick one you already have on the Sorting rules screen: ${installed.join(', ')}.`
        : `"${model}" is not installed, and Ollama reports no models at all. `
          + `Run "ollama pull ${model}" to download it.`,
    );
  }
  if (!res.ok) {
    // Ollama puts the actual reason in the body, and it is usually the only
    // thing that helps: "unknown model architecture: 'mllama'" says the
    // installed Ollama is too old for that model, which no status code
    // could ever convey. Reporting the bare number sent someone hunting a
    // problem in their prompt when the model had failed to load at all.
    let detail = '';
    try {
      const body = await res.json();
      if (typeof body?.error === 'string' && body.error.trim()) {
        // These arrive multi-line and repeat themselves; the first line
        // carries the reason.
        detail = body.error.split('\n')[0].trim();
      }
    } catch {
      /* no readable body — fall back to the status alone */
    }
    throw new NlRulesError(detail
...
module.exports = {
  NlRulesError, DEFAULT_HOST, DEFAULT_MODEL, draftRule, extractRuleLine, cloudHint, buildPrompt, listModels,
};
