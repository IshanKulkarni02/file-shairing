'use strict';

/**
 * Sorting rules: a small, human-written text format, parsed, evaluated, and
 * — when git is available — auto-committed to a tiny dedicated repo every
 * time it changes.
 *
 * Rules are the opposite of the file index on every count that made the
 * index a database instead of a text file: small, human-meaningful, change
 * rarely, contain nothing private, and benefit enormously from history —
 * "why has everything gone to /Drone since Tuesday?" is a `git log`, not an
 * investigation. That is what this file exists to make true.
 *
 * One rule per line:
 *
 *   when camera.make = "DJI"                 -> /Drone/{year}/{month}
 *   when kind = video and gps near "Manali"  -> /Rides/Manali
 *
 * Ordered, first match wins. The condition grammar is deliberately small —
 * ANDed clauses, optionally ORed together (disjunctive normal form, no
 * parentheses) — because every example this format needs to express is
 * exactly that shape, and a fully general boolean expression parser is a
 * lot of surface for a format meant to stay readable by a human who is not
 * a programmer.
 */

const fs = require('fs');
const path = require('path');
const crypto = require('crypto');
const { spawnSync } = require('child_process');
const { haversineKm } = require('./index-db');
const P = require('./paths');

const { normalizeRootPath } = P;

class SortRulesError extends Error {}

/**
 * A save refused because the rules changed underneath the editor.
 *
 * Deliberately extends SortRulesError so every existing handler keeps
 * working: anything that already treats a SortRulesError as "the person's
 * input was not usable" will report this sensibly rather than crashing. The
 * HTTP route checks for this narrower type first, so it can answer 409 and
 * hand back the current text instead of a generic 400.
 */
class RulesConflictError extends SortRulesError {
  constructor(message, { currentText = '', currentVersion = null } = {}) {
    super(message);
    this.currentText = currentText;
    this.currentVersion = currentVersion;
  }
}

const FIELDS = ['camera.make', 'camera.model', 'kind', 'date', 'trip'];
const DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;
/**
 * `camera` is the one that turns "by trip, then day, then camera" from a
 * rule per trip *per camera* into a single rule per trip. It resolves to the
 * camera model where there is one (which is what actually tells a DJI drone
 * apart from a DJI action camera — both report the make "DJI"), falling back
 * to the make, and to "Unknown camera" for a file that carries neither.
 *
 * `make` and `model` are exposed separately because the model alone is often
 * a part number rather than a name — "FC3582" means nothing to anyone
 * reading a folder list — so someone who wants a readable tree can still
 * write an explicit rule per camera with a name they chose.
 *
 * `trip` is Phase O1's own: it resolves through `file.trip`, which
 * lib/sort-engine.js sets per file before evaluation by looking up the
 * file's *approved* trip cluster (see lib/index-db.js's
 * approvedTripForHash()) — never a stored column, so it survives the very
 * move a trip-triggered rule causes. A file with no approved trip yet
 * substitutes "Unsorted", the same honest-fallback shape {camera} already
 * has in "Unknown camera".
 */
const PLACEHOLDERS = ['year', 'month', 'day', 'camera', 'make', 'model', 'trip'];
const DEFAULT_NEAR_RADIUS_KM = 20;

const RULES_SUBDIR = path.join('.lanshare', 'rules');
const RULES_FILENAME = 'rules.txt';

function rulesDir(library) {
  return path.join(library, RULES_SUBDIR);
}

function rulesPath(library) {
  return path.join(rulesDir(library), RULES_FILENAME);
}

// ---------------------------------------------------------------------------
// Parsing
// ---------------------------------------------------------------------------

function parseClause(text, lineNo) {
  const nearMatch = text.match(/^gps\s+near\s+"([^"]*)"(?:\s+within\s+(\d+(?:\.\d+)?)\s*km)?$/i);
  if (nearMatch) {
    const place = nearMatch[1].trim();
    if (!place) throw new SortRulesError(`Line ${lineNo}: "gps near" needs a place name in quotes`);
    return { field: 'gps', op: 'near', place, radiusKm: nearMatch[2] ? Number(nearMatch[2]) : DEFAULT_NEAR_RADIUS_KM };
  }

  const eqMatch = text.match(/^([a-zA-Z.]+)\s*=\s*(?:"([^"]*)"|(\S+))$/);
  if (eqMatch) {
    const field = eqMatch[1].toLowerCase();
    const value = eqMatch[2] !== undefined ? eqMatch[2] : eqMatch[3];
    if (!FIELDS.includes(field)) {
      throw new SortRulesError(`Line ${lineNo}: unknown field "${eqMatch[1]}" — expected one of ${FIELDS.join(', ')}`);
    }
    if (!value) throw new SortRulesError(`Line ${lineNo}: "${field}" needs a value`);
    if (field === 'date' && !DATE_PATTERN.test(value)) {
      throw new SortRulesError(`Line ${lineNo}: "date" needs a plain YYYY-MM-DD value — got "${value}"`);
    }
    return { field, op: '=', value };
  }

  throw new SortRulesError(`Line ${lineNo}: could not understand the condition "${text}"`);
}

/** Disjunctive normal form: an array of AND-groups, any one of which matching is enough. */
function parseCondition(text, lineNo) {
  if (!text.trim()) throw new SortRulesError(`Line ${lineNo}: a rule needs a condition`);
  return text.split(/\s+or\s+/i).map((group) => {
    const clauses = group.trim().split(/\s+and\s+/i).map((part) => parseClause(part.trim(), lineNo));
    return clauses;
  });
}

function parseDestination(text, lineNo) {
  const trimmed = text.trim();
  if (!trimmed.startsWith('/')) {
    throw new SortRulesError(`Line ${lineNo}: the destination must be an absolute library path starting with "/" — got "${trimmed}"`);
  }
  for (const match of trimmed.matchAll(/\{([^}]*)\}/g)) {
    if (!PLACEHOLDERS.includes(match[1])) {
      throw new SortRulesError(`Line ${lineNo}: unknown placeholder "{${match[1]}}" — expected one of ${PLACEHOLDERS.map((p) => `{${p}}`).join(', ')}`);
    }
  }
  // Stored in canonical form rather than as typed, because a destination is
  // a *library* path and the two readings of one otherwise disagree in a way
  // that loses files.
  //
  // "/../elsewhere" read as a library path means "/elsewhere" — an absolute
  // path cannot climb above its own root. But lib/sort-engine.js turns a
  // destination into a real one with path.join(library, ...segments), and
  // path.join has no such root to clamp against: it happily walks ".." up
  // out of the library and moves files somewhere the app can no longer see
  // them. The batch record written afterwards stores the collapsed path, so
  // undo could not bring them back either — silent, permanent data loss
  // from one typo.
  //
  // Normalizing here means the escaping spelling never reaches the engine at
  // all: every consumer sees "/elsewhere", which is both what the person
  // meant and safe by construction, since posix normalize clamps an
  // absolute path at its root.
  //
  // Safe to do with the placeholders still in place — {year}/{month}/{day}
  // only ever expand to digits or "Unknown" (see substitute), so a template
  // that is clean here cannot become unclean later, and normalize treats the
  // braces as ordinary path characters.
  //
  // normalizeRootPath is the same helper account roots already use, so a
  // rule destination and an account root agree on exactly what a library
  // path means rather than each having their own near-miss idea of it.
  const normalized = normalizeRootPath(trimmed);
  if (!normalized) {
    throw new SortRulesError(`Line ${lineNo}: the destination "${trimmed}" is not a valid library path`);
  }
  return normalized;
}

/**
 * The whole rules file, in order. Throws SortRulesError on the first
 * problem, naming the line — a rule file is short enough that "which line"
 * is always a useful answer, and refusing to save a broken file (see
 * saveRulesText below) is only worth anything if the error says why.
 */
function parse(text) {
  const rules = [];
  const lines = String(text || '').split(/\r?\n/);

  for (let i = 0; i < lines.length; i++) {
    const lineNo = i + 1;
    const trimmed = lines[i].trim();
    if (!trimmed || trimmed.startsWith('#')) continue;

    if (!/^when\s+/i.test(trimmed)) {
      throw new SortRulesError(`Line ${lineNo}: expected a rule starting with "when" — got "${trimmed}"`);
    }
    const body = trimmed.replace(/^when\s+/i, '');
    const arrowMatch = body.match(/^(.*?)\s*(?:->|→)\s*(.+)$/);
    if (!arrowMatch) {
      throw new SortRulesError(`Line ${lineNo}: expected "-> /destination" somewhere after the condition — got "${trimmed}"`);
    }

    rules.push({
      line: lineNo,
      raw: trimmed,
      conditionGroups: parseCondition(arrowMatch[1], lineNo),
      destination: parseDestination(arrowMatch[2], lineNo),
    });
  }
  return rules;
}

// ---------------------------------------------------------------------------
// Evaluation
// ---------------------------------------------------------------------------

function fieldValue(clause, file) {
  if (clause.field === 'camera.make') return file.cameraMake;
  if (clause.field === 'camera.model') return file.cameraModel;
  if (clause.field === 'kind') return file.kind;
  if (clause.field === 'trip') return file.trip;
  return null;
}

/** A file's capture date as a plain "YYYY-MM-DD" in local time, or null — a whole calendar day, not an instant. */
function capturedDateOnly(file) {
  if (!file.capturedAt) return null;
  const parsed = new Date(file.capturedAt);
  if (Number.isNaN(parsed.getTime())) return null;
  return `${parsed.getFullYear()}-${String(parsed.getMonth() + 1).padStart(2, '0')}-${String(parsed.getDate()).padStart(2, '0')}`;
}

/**
 * `geocodePlace(name)` is a synchronous lookup — `{lat, lon}` or null —
 * because every place a rule set could reference has to be resolved before
 * files are matched against it one at a time; see lib/sort-engine.js, which
 * pre-resolves every place with lib/geocode.js (async, network-backed) and
 * hands this a plain function over the results.
 */
function clauseMatches(clause, file, geocodePlace) {
  if (clause.op === '=' && clause.field === 'date') {
    return capturedDateOnly(file) === clause.value;
  }
  if (clause.op === '=') {
    const actual = fieldValue(clause, file);
    return actual != null && String(actual).toLowerCase() === clause.value.toLowerCase();
  }
  if (clause.op === 'near') {
    if (!Number.isFinite(file.gpsLat) || !Number.isFinite(file.gpsLon)) return false;
    const point = geocodePlace?.(clause.place);
    if (!point) return false;
    return haversineKm(point.lat, point.lon, file.gpsLat, file.gpsLon) <= clause.radiusKm;
  }
  return false;
}

function conditionMatches(conditionGroups, file, geocodePlace) {
  return conditionGroups.some((andGroup) => andGroup.every((clause) => clauseMatches(clause, file, geocodePlace)));
}

function pad2(n) {
  return String(n).padStart(2, '0');
}

function substitute(template, file) {
  if (!template.includes('{')) return template;
  const date = file.capturedAt ? new Date(file.capturedAt) : null;
  const valid = date && !Number.isNaN(date.getTime());

  // Camera fields come from EXIF, which means they come from a file someone
  // uploaded — so they are untrusted text on its way to becoming a folder
  // name. safeName() is the same guard every other user-supplied path
  // segment in this app goes through; without it a crafted camera model
  // could put separators or reserved names into the destination.
  const clean = (value, fallback) => {
    const safe = value ? P.safeName(String(value)) : '';
    return safe || fallback;
  };

  return template.replace(/\{(year|month|day|camera|make|model|trip)\}/g, (_, key) => {
    if (key === 'make') return clean(file.cameraMake, 'Unknown camera');
    if (key === 'model') return clean(file.cameraModel, 'Unknown camera');
    if (key === 'camera') {
      // Model first: it is what separates a DJI drone from a DJI action cam.
      return clean(file.cameraModel, clean(file.cameraMake, 'Unknown camera'));
    }
    if (key === 'trip') return clean(file.trip, 'Unsorted');
    if (!valid) return 'Unknown';
    if (key === 'year') return String(date.getFullYear());
    if (key === 'month') return pad2(date.getMonth() + 1);
    return pad2(date.getDate());
  });
}

/** The first matching rule's destination for this one file, or null if nothing matches. */
function destinationFor(rules, file, geocodePlace) {
  for (const rule of rules) {
    if (conditionMatches(rule.conditionGroups, file, geocodePlace)) {
      return substitute(rule.destination, file);
    }
  }
  return null;
}

/** Every distinct place name referenced by "gps near" clauses, so the caller can resolve them all before evaluating any file. */
function placesReferencedBy(rules) {
  const places = new Set();
  for (const rule of rules) {
    for (const group of rule.conditionGroups) {
      for (const clause of group) {
        if (clause.op === 'near') places.add(clause.place);
      }
    }
  }
  return [...places];
}

// ---------------------------------------------------------------------------
// Storage, versioned in git when git is available
// ---------------------------------------------------------------------------

function isGitAvailable() {
  try {
    return spawnSync('git', ['--version'], { encoding: 'utf8' }).status === 0;
  } catch {
    return false;
  }
}

function runGit(cwd, args) {
  try {
    return spawnSync('git', args, { cwd, encoding: 'utf8' }).status === 0;
  } catch {
    return false;
  }
}

function readRulesText(library) {
  try {
    return fs.readFileSync(rulesPath(library), 'utf8');
  } catch {
    return '';
  }
}

/**
 * A short token identifying exactly what the rules file says right now.
 *
 * Derived from the content rather than from git, because git is optional
 * here (see saveRulesText) and a version that vanishes when git is missing
 * would make the conflict check silently stop working on precisely the
 * machines least likely to notice. An empty file has a stable version too,
 * so a first-ever save is an ordinary compare rather than a special case.
 */
function rulesVersion(library) {
  return crypto.createHash('sha256').update(readRulesText(library), 'utf8').digest('hex').slice(0, 16);
}

/**
 * Save the rules text — parsed first, so a broken rule is refused before
 * anything on disk changes — and, when git is available, commit it to a
 * small dedicated repo holding only this one file. A missing git, or a
 * commit that fails for any reason (nothing changed, no git identity
 * configured anywhere on the machine), never blocks the save itself:
 * history here is an audit trail worth having, not a requirement the
 * feature depends on.
 */
function saveRulesText(library, text, { message = 'Update sorting rules', expectedVersion = null } = {}) {
  const parsed = parse(text); // throws SortRulesError on the first problem

  // Optimistic concurrency. Two people — or, once Phase O lands, a person and
  // the assistant — can both load the rules, both edit, and both save; the
  // second write would otherwise silently erase the first and report success.
  // Node being single-threaded does not help: the read and the write are
  // separate requests with a human-length gap between them.
  //
  // Callers that pass no expectedVersion are unchanged and overwrite as
  // before; that keeps internal callers (Phase K's import) working, while
  // every editing UI sends one and gets the check.
  if (expectedVersion !== null && expectedVersion !== undefined) {
    const current = rulesVersion(library);
    if (current !== expectedVersion) {
      throw new RulesConflictError(
        'These rules changed somewhere else while you were editing. '
        + 'Nothing was overwritten — reload to see the current rules, then reapply your change.',
        { currentText: readRulesText(library), currentVersion: current },
      );
    }
  }

  const dir = rulesDir(library);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(rulesPath(library), text, 'utf8');

  if (isGitAvailable()) {
    if (!fs.existsSync(path.join(dir, '.git'))) {
      runGit(dir, ['init', '-q']);
      // Local to this one repo, never --global — this identity is LANShare's
      // own, not the person's, and must never touch their real git config.
      runGit(dir, ['config', 'user.email', 'lanshare@localhost']);
      runGit(dir, ['config', 'user.name', 'LANShare']);
    }
    runGit(dir, ['add', RULES_FILENAME]);
    runGit(dir, ['commit', '-q', '-m', message]);
  }

  return parsed;
}

/** Recent history of the rules file, newest first — [] if there is none yet, or no git. */
function ruleHistory(library, { limit = 50 } = {}) {
  const dir = rulesDir(library);
  if (!fs.existsSync(path.join(dir, '.git'))) return [];
  const result = spawnSync('git', ['log', `-${limit}`, '--format=%H%x1f%aI%x1f%s'], { cwd: dir, encoding: 'utf8' });
  if (result.status !== 0 || !result.stdout.trim()) return [];
  return result.stdout.trim().split('\n').map((line) => {
    const [hash, date, subject] = line.split('\x1f');
    return { hash, date, subject };
  });
}

module.exports = {
  SortRulesError,
  RulesConflictError,
  rulesVersion,
  FIELDS,
  PLACEHOLDERS,
  DEFAULT_NEAR_RADIUS_KM,
  parse,
  destinationFor,
  placesReferencedBy,
  isGitAvailable,
  readRulesText,
  saveRulesText,
  ruleHistory,
  rulesPath,
};
