...
const TOOLS = {
  ...
  // No run/preview/execute — lib/assistant.js's loop intercepts this by
  // name before it ever reaches invokeTool(), because answering it means
  // pausing the loop and handing control back to the person, not producing
  // a tool result to feed back to the model. Listed here anyway so it is
  // still part of the schema the model sees and can choose to call.
  ask_user: {
    category: 'ask_user',
    description: 'Ask the person a clarifying question when the instruction is not clear enough to act on safely. '
      + 'Use this instead of guessing.',
    parameters: {
      type: 'object',
      properties: {
        question: { type: 'string' },
        options: { type: 'array', items: { type: 'string' }, description: 'A short list of concrete choices, if there is a natural small set.' },
      },
      required: ['question'],
    },
  },
...
async function invokeTool(name, args, ctx, trustLevel = 'ask') {
  ...
  if (def.category === 'ask_user') {
    throw new AssistantToolError('ask_user must be intercepted by the conversation loop, not invoked as an ordinary tool');
  }
