...
/**
 * Canonical form for an account root path (lib/accounts.js): a leading
 * slash, no trailing slash except for "/" itself, no redundant segments.
 * Returns null for anything unsafe (traversal, not absolute).
 *
 * This must produce exactly the same string shape that resolveSafe()'s
 * `rel` does for the same folder — isWithinRoots() in lib/permissions.js
 * compares a root against a resolved path with `===` and a `root + '/'`
 * prefix check. A root stored as "/Family/" would never equal the "/Family"
 * that resolveSafe() returns for that same folder, and the prefix check
 * fails too since "/Family" is shorter than "/Family/" — silently locking
 * an account out of its own root the moment someone types a trailing slash
 * into the "restrict to albums" field.
 */
function normalizeRootPath(root) {
  const raw = String(root == null ? '' : root).replace(/\\/g, '/');
...
/**
 * Claim a free name, atomically.
 *
 * `uniqueName` below only *looks*, which is fine when one thing is running.
 * Uploads are not: a phone sending a batch of photos opens several requests
 * at once, and two of them holding the same filename would each see it free,
 * each pick it, and each write to it — losing all but one, and erroring on
 * the way.
 *
 * Creating the file with 'wx' is the fix, because exclusive create is atomic:
 * exactly one caller can win a given name, and the loser moves on to the next
 * candidate. It leaves an empty placeholder holding the name, so the caller
 * must remove it if the write then fails.
 */
function reserveName(fs, dir, name, { maxAttempts = 5000 } = {}) {
  const e = extOf(name);
  const stem = name.slice(0, name.length - e.length);

  for (let n = 1; n <= maxAttempts; n++) {
    const candidate = n === 1 ? name : `${stem} (${n})${e}`;
    try {
      fs.closeSync(fs.openSync(path.join(dir, candidate), 'wx'));
      return candidate;
    } catch (err) {
      if (err.code !== 'EEXIST') throw err;
    }
  }
  throw new Error(`Could not find a free name for "${name}" after ${maxAttempts} tries`);
}

/** The extension, using the same rule uniqueName has always used. */
function extOf(name) {
 ...
 * prefix test says "/library-backup" is inside "/library" — which is exactly
 * how a containment check ends up letting a path escape.
 */
