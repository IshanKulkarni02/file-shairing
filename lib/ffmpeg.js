...
function candidates(name) {
  ...
  const list = [];
  // 0. Unpacked by lib/runtime.js when running as a pkg-packaged executable.
  ...
  // 0b. electron-builder's extraResources land here in a packaged desktop
  // build — process.resourcesPath is Electron's own authoritative answer for
  // "where are my resources", true in both a packaged app and a dev run.
  if (process.versions?.electron && process.resourcesPath) {
    list.push(path.join(process.resourcesPath, 'ffmpeg', exe));
  }
