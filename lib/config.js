...
/**
 * A save refused because config.json changed underneath the caller — the
 * same shape as lib/sort-rules.js's RulesConflictError, for the same
 * reason: two processes (the desktop app and a background Discovery pass,
 * say) can both read config, both compute a change, and both save, and
 * without a version check the second write silently erases the first.
 * plan.md flagged this debt explicitly when the rules-file version of this
 * bug was fixed; Phase O2's trust-state writes are the first caller that
 * actually needs it.
 */
class ConfigConflictError extends Error {
  constructor(message, { currentConfig = null, currentVersion = null } = {}) {
    super(message);
...
const DEFAULTS = {
  ...
  closeToTray: true,
  startOnLogin: false,
  // Phase O2: per-action-type trust level ('ask' | 'ghost' | 'auto'), keyed
  // by action type (e.g. 'trip_cluster', 'camera_correction'). Absent means
  // 'ask' — see lib/trust.js, which owns every rule about what these values
  // mean and how they change; this file only stores and version-checks them.
  trust: {},
};
 ...
 */
/**
 * A short token identifying exactly what's on disk right now — the same
 * derive-from-content shape as lib/sort-rules.js's rulesVersion(), and for
 * the same reason: it has to stay meaningful even before anything has ever
 * called save() with a version check, and a missing file gets a stable
 * version too, so a first-ever versioned write is an ordinary compare
...
/**
 * Callers that omit `expectedVersion` are unchanged and overwrite as
 * before — that keeps every existing caller (accounts, sessions, setUser)
 * working exactly as it always has; only a caller that actually needs the
 * check (lib/trust.js's trust-state writes) passes one.
 */
function save(config, { expectedVersion = null } = {}) {
  if (expectedVersion !== null && expectedVersion !== undefined) {
    const current = configVersion();
    if (current !== expectedVersion) {
      throw new ConfigConflictError(
        'The configuration changed somewhere else while this was being written. Nothing was overwritten.',
        { currentConfig: load(), currentVersion: current },
      );
    }
  }
