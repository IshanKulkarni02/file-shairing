...
/**
 * Revoke every session for a user. Used when an account is disabled or
 * deleted.
 *
 * Compares case-insensitively, matching how usernames are compared
 * everywhere else (lib/accounts.js, lib/auth.js) — a caller passing the
 * username in a different case than it was stored (e.g. a URL parameter)
 * must still find the same sessions, or a delete/disable from a differently
 * cased route silently leaves live-looking session records behind for an
...
function revokeAllForUser(username, exceptId = null) {
  ...
  let changed = false;
  for (const [id, record] of map) {
    if (record.username.toLowerCase() === target && id !== exceptId) {
      map.delete(id);
      changed = true;
    }
  }
...
function list(username = null) {
  ...
  const target = username ? username.toLowerCase() : null;
  const filtered = target ? all.filter((r) => r.username.toLowerCase() === target) : all;
