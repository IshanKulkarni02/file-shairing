...
async function moveLibrary(oldPath, newPath, mode = 'move') {
  // path.resolve() throws a raw TypeError on anything but a string, which
  // would surface to the desktop UI as Electron's "Error invoking remote
  // method '...'" wrapper rather than a message fit to show someone — worth
  // one check here since the caller only guards this on the happy path.
  if (typeof newPath !== 'string' || !newPath.trim()) {
    throw new LibraryError('Choose a folder first.');
  }
  if (mode !== 'move' && mode !== 'point') {
    throw new LibraryError(`Unknown move mode: ${mode}`);
  }
