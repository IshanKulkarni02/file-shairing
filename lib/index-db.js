...
const SCHEMA = `
  ...
  -- Phase O3: accepted/corrected instruction memory. Every rule instruction
  -- the assistant drafted and a human accepted, or corrected, kept as the
  -- corrected PAIR (the instruction, and the rule actually wanted) — never
  -- as "the model said X and X was wrong," which anchors a model toward
  -- repeating its own mistake rather than the fix. See lib/assistant-memory.js.
  ...
  CREATE INDEX IF NOT EXISTS idx_assistant_memory_created_at ON assistant_memory(created_at);
`;
...
class IndexDb {
  constructor(dbPath) {
    ...
    this._insertMemory = this.db.prepare(`
      INSERT INTO assistant_memory (id, instruction, rule_text, is_correction, created_at)
      VALUES (:id, :instruction, :ruleText, :isCorrection, :createdAt)
    `);
    this._allMemory = this.db.prepare('SELECT * FROM assistant_memory ORDER BY created_at DESC');
  }
  ...
  /** Record one instruction/rule pair — accepted as-is, or the corrected pair after a rewrite. See lib/assistant-memory.js for retrieval. */
  rememberInstruction({ instruction, ruleText, isCorrection = false }) {
    const id = crypto.randomUUID();
    this._insertMemory.run({
      id, instruction, ruleText, isCorrection: isCorrection ? 1 : 0, createdAt: new Date().toISOString(),
    });
    return id;
  }

  /** Every remembered instruction/rule pair, newest first — small enough at personal scale to rank in JS rather than in SQL. */
  allMemory() {
    return this._allMemory.all().map((row) => ({
      id: row.id, instruction: row.instruction, ruleText: row.rule_text,
      isCorrection: Boolean(row.is_correction), createdAt: row.created_at,
    }));
  }
