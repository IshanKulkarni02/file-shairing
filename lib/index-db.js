...
class IndexDb {
  constructor(dbPath) {
    ...
    this._filesForClustering = this.db.prepare(`
      SELECT rel_path, hash, kind, camera_make, camera_model,
             captured_at, captured_at_basis, gps_lat, gps_lon
      FROM files
  ...
  /** Every hashed, dated, non-encrypted file — what lib/trip-clustering.js pools and sorts by capture time. */
  filesForClustering() {
    return this._filesForClustering.all();
  }
  ...
  /** The approved correction, if any, covering this camera at this instant — used by lib/sort-engine.js's correctedCapturedAt() resolver. */
  approvedCorrectionFor(cameraMake, cameraModel, capturedAt) {
    return this._approvedCorrectionFor.get({ cameraMake, cameraModel, capturedAt }) || null;
  }
