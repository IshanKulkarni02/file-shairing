...
/**
 * `pull` is one-way in the other direction from `mirror`: bring what is on the
 * drive into the library and **never write to the drive at all**. It is the
 * right answer for a Google Drive or Dropbox folder you want to collect from
 * rather than manage — mirror would push your library onto it and delete
 * whatever it holds that your library does not, which for a shared cloud
 * folder is somewhere between rude and catastrophic.
 *
 * It also never deletes in the library. A pull is "fetch what is there", and
 * a file disappearing from the cloud is not an instruction to lose your copy.
 */
const POLICIES = ['keep-both', 'newest-wins', 'mirror', 'pull'];
...
function buildPlan({
  ...
  for (const rel of [...paths].sort()) {
    ...
    // Collect from the other side and touch nothing over there. No deletions
    // in either direction: a file that vanished from a cloud folder is not an
    // instruction to lose the copy you fetched.
    if (policy === 'pull') {
      if (t && !sameFile(s, t)) {
        actions.push({ type: 'copy', direction: 'to-source', path: rel, reason: s ? 'newer there' : 'new there' });
      }
      continue;
    }
