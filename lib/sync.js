...
async function preview({ library, sourceDir, targetDir, driveRoot, targetId, policy, conflictLabel, now = new Date() }) {
  ...
  // One folder inside the other is a loop: every run copies the previous
  // run's copy, and the library grows until the disk is full — unattended,
  // because sync-on-connect keeps doing it. Locations refuse to be registered
  // inside the library, which closes the route people can reach; this closes
  // it for anything calling the engine directly, including a future caller
  // that has not learned the rule yet.
  const sourceRoot = path.resolve(sourceDir);
  const targetRoot = path.resolve(targetDir);
  if (sourceRoot === targetRoot) {
    throw new SyncError('That sync copies a folder to itself, which would do nothing');
  }
  if (isInside(sourceRoot, targetRoot) || isInside(targetRoot, sourceRoot)) {
    throw new SyncError(
      'One of those folders is inside the other, so syncing them would copy a folder '
      + 'into itself over and over. Choose somewhere outside it.',
    );
  }
