...
function createApp(config, { secrets = null } = {}) {
  ...
  app.get('/api/sort-rules', permissions.requireRole('admin'), (req, res) => {
    ...
    res.json({
      text, ruleCount, error, gitAvailable: sortRules.isGitAvailable(), history: sortRules.ruleHistory(LIBRARY),
    });
  });
  ...
  app.post('/api/sort-rules', permissions.requireRole('admin'), (req, res) => {
    ...
    try {
      const parsed = sortRules.saveRulesText(LIBRARY, text, { message });
      return res.json({ ok: true, ruleCount: parsed.length });
    } catch (err) {
