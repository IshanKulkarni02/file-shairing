...
function createApp(config, { secrets = null } = {}) {
  ...
  app.get('/api/sort-rules/plan', permissions.requireRole('admin'), async (req, res) => {
    try {
      ...
      const result = await sortEngine.plan({ library: LIBRARY, entries });
      res.json(result);
    } catch (err) {
  ...
  app.post('/api/sort-rules/apply', permissions.requireRole('admin'), async (req, res) => {
    try {
      ...
      // reindexed.
      const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
      const planned = await sortEngine.plan({ library: LIBRARY, entries });
  ...
  app.post('/api/sort-rules/draft', permissions.requireRole('admin'), async (req, res) => {
    ...
    try {
      ...
      if (draft.parsed) {
        const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
        preview = await sortEngine.plan({ library: LIBRARY, entries, rulesText: draft.text });
      }
  ...
  app.post('/api/sort-rules/run-once', permissions.requireRole('admin'), async (req, res) => {
    ...
    try {
      const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
      const planned = await sortEngine.plan({ library: LIBRARY, entries, rulesText: text });
      const batch = await sortEngine.apply({ library: LIBRARY, moves: planned.moves });
      runIndexScan();
      res.json(batch);
    } catch (err) {
