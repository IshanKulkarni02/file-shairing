...
function createApp(config) {
  ...
  async function openForRead(req, res, rawPath) {
    ...
    // definition — it has no key and never will. Only the browser that
    // created them can read them, so any route that would need plaintext
    // (thumbnails, transcoding, previews) has to decline rather than fail
  ...
  app.get('/api/list', async (req, res) => {
    ...
    await Promise.all(entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) {
        ...
        const childVault = vaults.readVaultMeta(abs);
        folders.push({
          name: entry.name,
          path: relPath,
          isDir: true,
    ...
    bb.on('file', (field, stream, info) => {
      ...
      const job = (async () => {
        ...
        saved.push({
          ...
          size: encrypting ? Number(req.headers['content-length']) || stat.size : stat.size,
          mtime: stat.mtimeMs,
        ...
        stream.resume();
        // A failed encrypted write must not leave a half-written .part
        // behind that a later upload could trip over.
        await fsp.rm(path.join(destDir, `${name}.part`), { force: true }).catch(() => {});
      });
