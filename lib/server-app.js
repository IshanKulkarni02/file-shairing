...
const { IndexDb } = indexDbLib;
const indexer = require('./indexer');
const federation = require('./federation');
const centralIndexLib = require('./central-index');
...
const contentIndexLib = require('./content-index');
const patternDiscovery = require('./pattern-discovery');
const trustLib = require('./trust');
const assistant = require('./assistant');
const assistantMemory = require('./assistant-memory');
...
function createApp(config, { secrets = null } = {}) {
  const LIBRARY = path.resolve(config.library);
  const PUBLIC_DIR = path.join(__dirname, '..', 'public');

  // Lets /api/search reach other paired machines: connections.connect() needs
  // a place to decrypt a stored password from, and that place is Electron's
  // OS keychain via safeStorage, which only the desktop app has. A headless
  // `node server.js` — and every test in this repo — passes nothing here, so
  // federation quietly stays local-only rather than failing, the same
  // "unavailable, so not stored" degradation lib/connections.js already uses.
  const secretsStore = secrets && secrets.available ? secrets : { available: false };
  ...
  // Content-search embeddings (Phase N) — same in-flight tracking as
  // runIndexScan() above, but deliberately never called at startup: unlike
  // the metadata scan, this is a large one-time model download plus real
  // per-image compute, and nobody should pay either cost without asking for
  // it. Its own presence in content_embeddings *is* the opt-in — there is
  // no separate enabled/disabled flag in config to drift out of sync with
  // what has actually been built.
  let contentScan = null;
  let lastContentReport = null;
  function runContentScan() {
    if (!contentScan) {
      contentScan = contentIndexLib.buildContentIndex({ library: LIBRARY, db: indexDb })
        .then((report) => { lastContentReport = { ...report, finishedAt: new Date().toISOString() }; return report; })
        .catch((err) => {
          lastContentReport = { error: err.message, finishedAt: new Date().toISOString() };
          console.warn(`[content-index] build failed: ${err.message}`);
        })
        .finally(() => { contentScan = null; });
    }
    return contentScan;
  }

  /**
   * Whether `req.account` may see this row at all — the same two checks
   * GET /api/search already applied to every raw row before showing it:
   * root-scoping, and a locked server-unlock vault hiding even filenames.
   * Shared with GET /api/search/content below so a security-relevant filter
   * has exactly one place to be correct, not two copies that could drift.
   */
  function visibleRow(req, row) {
    if (!permissions.isWithinRoots(req.account, row.rel_path)) return false;
    if (row.encrypted) {
      const ctx = vaults.contextFor(LIBRARY, row.rel_path);
      if (ctx && ctx.type !== 'e2e' && !ctx.unlocked) return false;
    }
    return true;
  }

  // The central index's two derived keys, cached after the first successful
  // derivation. Deriving them is a deliberately slow PBKDF2 call (the same
  // cost a vault passphrase pays) — worth paying once per server run, not
  // on every search or every publish.
  let centralIndexKeys = null;
  let lastCentralPublish = null;
  async function getCentralIndexKeys() {
    if (centralIndexKeys) return centralIndexKeys;
    if (!config.centralIndex?.secret || !secretsStore.available) return null;
    const passphrase = secretsStore.decrypt(config.centralIndex.secret);
    if (!passphrase) return null;
    centralIndexKeys = await centralIndexLib.deriveKeys(passphrase);
   ...
   *
   * Every remote visitor deliberately shares the single 'over the internet'
   * bucket rather than being told apart, and that is not an oversight:
   *
   *  - There is nothing trustworthy to tell them apart *by*. lib/tunnel.js
   *    replays whatever headers the remote peer sent, so any per-client
   *    value here would be attacker-chosen — a guesser could rotate it and
   *    get unlimited login attempts, which is far worse than sharing one
   *    bucket. The relay cannot supply a trustworthy id either: once a room
   *    is paired it is a blind byte pipe (see relay/server.js) precisely so
   *    it can never read the traffic, and having it parse and stamp
   *    requests would trade this app's end-to-end guarantee for a rate
   *    limit.
   *  - Concurrent remote visitors are not actually a thing. The relay
   *    allows exactly one host and one client per room and refuses a second
   *    with "that role is taken", so at most one remote peer is connected
   *    at a time.
   *
   * What remains is that one remote peer burning its 10 attempts also
   * blocks the *next* remote peer for the rest of the window. That is the
   * intended direction to fail in: someone at the keyboard, and everyone on
   * the LAN, keep their own separate buckets and are unaffected.
   */
  ...
  app.get('/api/me', (req, res) => {
    ...
    res.json({
      ...
      // Whether GET /api/search/content has anything to actually search —
      // cheap to compute (an indexed COUNT), and lets the gallery decide
      // whether to offer content search at all without a role-gated round
      // trip first, since building the index is admin-only but using it
      // is not (mirrors /api/search's own openness to every role).
      contentSearchAvailable: indexDb.countEmbedded(clipLib.MODEL_ID) > 0,
    });
  });
  ...
  app.get('/api/search', async (req, res) => {
    ...
    const roots = req.account.roots || ['/'];
    const restricted = !roots.includes('/');
    const params = { text: q, kind, cameraMake: camera, from, to, near, radiusKm, limit };
    ...
    const rows = indexDb.search({ ...params, limit: restricted ? Math.min(limit * 5, 2000) : limit });

    // visibleRow mirrors /api/list: a locked server-unlock vault shows
    // nothing at all, not even filenames — so a match against a name inside
    // one must not leak that the file exists while it is locked. An e2e
    // vault has no server-side lock state to gate on (the server never
    // holds that key), so its entries — already content-blind, since the
    // indexer never opens vault files — pass through unconditionally.
    const localResults = [];
    for (const row of rows) {
      if (localResults.length >= limit) break;
      if (!visibleRow(req, row)) continue;
      localResults.push(indexDbLib.dbRowToResult(row));
    }

    // Federation reaches every paired machine using whatever credential that
    // pairing was set up with — potentially a broader identity than the
    // local account making this request — and the central index is reached
    // with a passphrase shared across every device, not scoped to any one
    // local account either. Running either for anyone below admin would let
    // a restricted local viewer transitively exercise a stored
    // admin-on-another-machine password, or read a passphrase-protected
    // index, that it was never given. Skipped, not refused: an ordinary
    // local search must keep working for every role, just without reaching
    // past this machine — which is also why this still goes through
    // federatedSearch with nothing configured rather than a separate code
    // path: the response stays exactly one shape regardless of who is
    // asking or how far the search actually reached.
    const isAdmin = req.account.role === 'admin';
    const { results: merged, peers, centralIndex } = await federation.federatedSearch({
      library: LIBRARY,
      config: isAdmin ? config : { ...config, connections: [], centralIndex: null },
      secrets: secretsStore,
      localResults,
      params,
      centralKeys: isAdmin ? await getCentralIndexKeys() : null,
    });

    res.json({ results: merged.slice(0, limit), peers, centralIndex, indexing: Boolean(indexScan) });
    return undefined;
  });

  /**
   * "Photos of whiteboards" — what a filename or EXIF field can never answer
   * (Phase N). Open to any role, exactly like GET /api/search above: content
   * search is a way of searching, not an administrative capability, so the
   * same root-scoping and vault-lock rules apply via visibleRow() and
   * nothing more is asked of the caller than an ordinary search already is.
   * Building the index that makes this useful, below, is a different
   * question and stays admin-only, same split as /api/index/rebuild vs
   * /api/search.
   */
  app.get('/api/search/content', async (req, res) => {
    ...
    const requestedLimit = Number(req.query.limit);
    const limit = Number.isFinite(requestedLimit) && requestedLimit > 0
      ? Math.min(Math.floor(requestedLimit), 200) : 60;

    // Nothing embedded yet means nothing to rank against — answered here,
    // before spending the (comparatively expensive) model call on a query
    // that could not possibly return anything.
    if (indexDb.countEmbedded(clipLib.MODEL_ID) === 0) {
      res.json({ results: [], available: false });
      return undefined;
    }

    try {
      const queryVector = await clipLib.embedText(q);
      // The visibility check is handed *into* the ranking rather than
      // applied to its output, because an embedding belongs to a content
      // hash and a hash can have copies in several albums. Filtering
      // afterwards would let a copy this account cannot see stand in for
      // one it can, and the result would vanish for no reason it could
      // observe — see searchByContent's own note. Doing it here also means
      // `limit` counts results the account will actually be shown, so no
      // over-fetch-and-trim is needed.
      const rows = indexDb.searchByContent(queryVector, {
        model: clipLib.MODEL_ID,
        limit,
        isVisible: (row) => visibleRow(req, row),
      });

      res.json({ results: rows.map(indexDbLib.dbRowToResult), available: true });
    } catch (err) {
      res.status(err instanceof clipLib.ClipError ? 503 : 500).json({ error: err.message });
    }
    return undefined;
  });

  app.post('/api/content-index/build', permissions.requireRole('admin'), (req, res) => {
    const alreadyRunning = Boolean(contentScan);
    runContentScan();
    res.json({ started: !alreadyRunning, alreadyRunning });
  });

  app.get('/api/content-index/status', permissions.requireRole('admin'), (req, res) => {
    res.json({
      building: Boolean(contentScan),
      lastReport: lastContentReport,
      embedded: indexDb.countEmbedded(clipLib.MODEL_ID),
      embeddable: indexDb.countEmbeddable(),
      modelCached: clipLib.isCached(),
    });
  });

  /**
   * Setting up the central index: where the relay is, and the passphrase
   * every device shares. The passphrase itself is never returned by any
   * route — only whether one is currently configured.
   */
  app.post('/api/central-index/setup', permissions.requireRole('admin'), async (req, res) => {
    const { relayHost, relayPort, passphrase, label } = req.body || {};
    if (!relayHost || !Number.isFinite(Number(relayPort))) {
      return res.status(400).json({ error: 'Enter the relay address and port' });
    }
    if (typeof passphrase !== 'string' || passphrase.length < 8) {
      return res.status(400).json({ error: 'A central index passphrase must be at least 8 characters' });
    }
    if (!secretsStore.available) {
      return res.status(400).json({
        error: 'This machine has no secure place to remember the passphrase (no OS keychain available), '
          + 'so the central index cannot be set up here.',
      });
    }

    config.centralIndex = {
      relayHost, relayPort: Number(relayPort),
      label: (label && label.trim()) || undefined,
      secret: secretsStore.encrypt(passphrase),
    };
    configLib.save(config);
    centralIndexLib.ensureDeviceId(config, configLib);
    centralIndexKeys = await centralIndexLib.deriveKeys(passphrase);
  ...
  app.get('/api/central-index/status', permissions.requireRole('admin'), (req, res) => {
    res.json({
      configured: Boolean(config.centralIndex?.secret),
      relayHost: config.centralIndex?.relayHost || null,
      relayPort: config.centralIndex?.relayPort || null,
      deviceId: config.deviceId || null,
      label: config.centralIndex ? centralIndexLib.deviceLabel(config) : null,
      lastPublish: lastCentralPublish,
    });
  });

  app.post('/api/central-index/publish', permissions.requireRole('admin'), async (req, res) => {
    if (!config.centralIndex?.relayHost) {
      return res.status(400).json({ error: 'Set up the central index first' });
    }
    const keys = await getCentralIndexKeys();
    if (!keys) {
      return res.status(400).json({
        error: 'The central index passphrase is not available on this session — set it up again here.',
      });
    }

    try {
      const deviceId = centralIndexLib.ensureDeviceId(config, configLib);
      const label = centralIndexLib.deviceLabel(config);
      const relay = { relayHost: config.centralIndex.relayHost, relayPort: config.centralIndex.relayPort };

      // Everything the local index knows, exactly as /api/search already
      // shows an admin — an encrypted file's shallow metadata (path, size,
      // hash; never its content) is no more exposed here than it already is
      // to any admin browsing this server directly.
      const rows = indexDb.search({ limit: 1_000_000 });
      const entries = rows.map(indexDbLib.dbRowToResult);

      const deviceBlob = centralIndexLib.buildDeviceBlob({ deviceId, label, entries });
      const sealedDevice = centralIndexLib.seal(keys.encryptionKey, centralIndexLib.deviceAad(deviceId), deviceBlob);
      await tunnel.storeRequest({
        ...relay, action: 'put', key: centralIndexLib.deviceSlotKey(keys.lookupKey, deviceId),
        blob: sealedDevice.toString('base64'),
      });

      const rosterKey = centralIndexLib.rosterSlotKey(keys.lookupKey);
      let existingRoster = null;
      try {
        const rosterMsg = await tunnel.storeRequest({ ...relay, action: 'get', key: rosterKey });
        if (rosterMsg.blob) {
          existingRoster = centralIndexLib.open(
            keys.encryptionKey, centralIndexLib.ROSTER_AAD, Buffer.from(rosterMsg.blob, 'base64'),
          );
        }
      } catch {
        // A roster that cannot be read yet (first publish ever, or a
        // transient relay hiccup) is treated as empty rather than failing
        // the publish — mergeRoster(null, ...) still produces a valid one.
      }
      const roster = centralIndexLib.mergeRoster(existingRoster, deviceId, label);
      const sealedRoster = centralIndexLib.seal(keys.encryptionKey, centralIndexLib.ROSTER_AAD, roster);
      await tunnel.storeRequest({ ...relay, action: 'put', key: rosterKey, blob: sealedRoster.toString('base64') });

      lastCentralPublish = { at: deviceBlob.publishedAt, entries: entries.length };
      res.json({ ok: true, entries: entries.length, publishedAt: deviceBlob.publishedAt });
    } catch (err) {
      res.status(502).json({ error: `Could not publish to the central index: ${err.message}` });
    }
    return undefined;
  });
  ...
  // ---------------------------------------------------------------------------
  // Sorting rules (Phase L) — admin-only throughout. A rule set decides where
  // files in the *whole* library land, which is exactly the kind of
  // library-wide, structural action already reserved for admin elsewhere
  // (locations, sync targets, accounts) rather than something scoped per
  // account root the way browsing and search are.
  // ---------------------------------------------------------------------------

  app.get('/api/sort-rules', permissions.requireRole('admin'), (req, res) => {
    const text = sortRules.readRulesText(LIBRARY);
    let ruleCount = 0;
    let error = null;
    try {
      ruleCount = sortRules.parse(text).length;
    } catch (err) {
      error = err.message;
    }
    // The version is an ETag in the ordinary HTTP sense: hand it back on save
    // and the server can tell "I edited the rules I was shown" from "I edited
    // a copy that has since been replaced".
    const version = sortRules.rulesVersion(LIBRARY);
    ...
    res.json({
      ...
      error,
      version,
      gitAvailable: sortRules.isGitAvailable(),
      history: sortRules.ruleHistory(LIBRARY),
    });
  });

  app.post('/api/sort-rules', permissions.requireRole('admin'), (req, res) => {
    const { text, message, version } = req.body || {};
    if (typeof text !== 'string') return res.status(400).json({ error: 'Send the rules as text' });

    // Accepted from the standard header or the body — the header is the
    // conventional spelling, the body is what a fetch() call finds easiest.
    // Absent means "overwrite", which keeps internal callers working; every
    // editor sends one.
    const ifMatch = (req.get('If-Match') || '').replace(/^"|"$/g, '');
    const expectedVersion = ifMatch || (typeof version === 'string' ? version : null);

    try {
      const parsed = sortRules.saveRulesText(LIBRARY, text, { message, expectedVersion });
      return res.json({ ok: true, ruleCount: parsed.length, version: sortRules.rulesVersion(LIBRARY) });
    } catch (err) {
      // Checked before the SortRulesError branch it inherits from, so a
      // conflict answers 409 with what is actually on disk rather than a
      // bare 400 that leaves the editor with nothing to reconcile against.
      if (err instanceof sortRules.RulesConflictError) {
      ...
      if (err instanceof sortRules.SortRulesError) return res.status(400).json({ error: err.message });
      throw err;
    }
  });

  /** Everything the current rules would do to the whole library, right now — read-only. */
  app.get('/api/sort-rules/plan', permissions.requireRole('admin'), async (req, res) => {
    try {
      const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
      const result = await sortEngine.plan({ library: LIBRARY, entries, db: indexDb });
      res.json(result);
    } catch (err) {
      const status = err instanceof sortRules.SortRulesError ? 400 : 500;
      res.status(status).json({ error: err.message });
    }
    return undefined;
  });

  app.post('/api/sort-rules/apply', permissions.requireRole('admin'), async (req, res) => {
    try {
      // Re-planned here rather than trusting a plan the client got earlier —
      // files can change between "show me the dry run" and "go ahead", and
      // applying a stale plan is exactly how a rule-driven move ends up
      // acting on a file that has since moved, been deleted, or been
      // reindexed.
      const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
      const planned = await sortEngine.plan({ library: LIBRARY, entries, db: indexDb });
      const batch = await sortEngine.apply({ library: LIBRARY, moves: planned.moves });
      runIndexScan(); // paths just changed under the index's feet
      res.json(batch);
    } catch (err) {
      const status = err instanceof sortRules.SortRulesError ? 400 : 500;
      res.status(status).json({ error: err.message });
  ...
  app.get('/api/sort-rules/batches', permissions.requireRole('admin'), (req, res) => {
    res.json({ batches: sortEngine.loadBatches(LIBRARY) });
  });

  app.post('/api/sort-rules/undo', permissions.requireRole('admin'), async (req, res) => {
    try {
      const result = await sortEngine.undoLastBatch({ library: LIBRARY });
      runIndexScan();
      res.json(result);
    } catch (err) {
      const status = err instanceof sortEngine.SortEngineError ? 400 : 500;
      res.status(status).json({ error: err.message });
    }
    return undefined;
  });
  ...
  /**
   * Draft a rule from a sentence, and — only when the draft actually parses
   * — a live preview of what it would do, in the same round trip. The
   * draft is never saved and never applied here; see /api/sort-rules (save
   * as a standing rule, reusing the same route a hand-typed rule set
   * already goes through) and /api/sort-rules/run-once below.
   */
  /**
   * Which models the local Ollama has, and which one drafting will use.
   *
   * The model was previously a hardcoded constant with no way to see or
   * change it from anywhere. Asking Ollama for a model it does not have
   * returns 404 — which reads like "server down" but only happens when the
   * server is up — so the whole feature looked broken with no route to
   * discovering why, or to picking one of the models actually installed.
   */
  app.get('/api/sort-rules/models', permissions.requireRole('admin'), async (req, res) => {
    const host = config.nlRules?.host || nlRules.DEFAULT_HOST;
    const installed = await nlRules.listModels({ host });
    const selected = config.nlRules?.model || nlRules.DEFAULT_MODEL;
    res.json({
      host,
      installed,
      selected,
      // Distinguishes "Ollama is not running" from "it is running but the
      // chosen model is missing" — two problems with completely different fixes.
      reachable: installed.length > 0,
      selectedInstalled: installed.includes(selected),
    });
  });

  app.post('/api/sort-rules/model', permissions.requireRole('admin'), (req, res) => {
    const { model } = req.body || {};
    if (typeof model !== 'string' || !model.trim()) {
      return res.status(400).json({ error: 'Pick a model' });
    }
    config.nlRules = { ...(config.nlRules || {}), model: model.trim() };
    configLib.save(config);
    return res.json({ ok: true, selected: config.nlRules.model });
  });

  app.post('/api/sort-rules/draft', permissions.requireRole('admin'), async (req, res) => {
    const { instruction } = req.body || {};
    try {
      const draft = await nlRules.draftRule({
        instruction,
        host: config.nlRules?.host || nlRules.DEFAULT_HOST,
        model: config.nlRules?.model || nlRules.DEFAULT_MODEL,
      });
      let preview = null;
      if (draft.parsed) {
        const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
        preview = await sortEngine.plan({ library: LIBRARY, entries, rulesText: draft.text, db: indexDb });
      }
      res.json({ ...draft, preview });
    } catch (err) {
      res.status(err instanceof nlRules.NlRulesError ? 400 : 500).json({ error: err.message });
    }
    return undefined;
  });

  /**
   * Apply a one-off instruction's draft without ever saving it as a
   * standing rule. Still recorded as an ordinary batch by sortEngine.apply
   * — "run once" is not "run once and hope", the same undo guarantee a
   * saved rule's apply gets applies here unchanged.
   */
  app.post('/api/sort-rules/run-once', permissions.requireRole('admin'), async (req, res) => {
    const { text } = req.body || {};
    if (typeof text !== 'string' || !text.trim()) return res.status(400).json({ error: 'Nothing to run' });
    try {
      const entries = indexDb.search({ limit: 1_000_000 }).map(indexDbLib.dbRowToResult);
      const planned = await sortEngine.plan({ library: LIBRARY, entries, rulesText: text, db: indexDb });
      const batch = await sortEngine.apply({ library: LIBRARY, moves: planned.moves });
      runIndexScan();
      res.json(batch);
    } catch (err) {
  ...
  // --- Phase O1: the Adaptive Pattern Engine's Ghost Mode review queue -------
  // Discovery itself never touches a media file — it only ever proposes.
  // Approving a proposal appends a rule (see lib/pattern-discovery.js); the
  // existing /api/sort-rules/plan and /apply routes above are what actually
  // move anything, exactly like a rule a person wrote by hand.

  /** Run one Discovery pass over the whole library, right now — applying whatever each action type's current trust level calls for. */
  app.post('/api/pattern-engine/scan', permissions.requireRole('admin'), async (req, res) => {
    try {
      const result = await patternDiscovery.runDiscoveryPass({ library: LIBRARY, db: indexDb, config });
      res.json({
        proposed: result.proposed.map((p) => p.proposal),
        autoAttached: result.autoAttached,
      });
    } catch (err) {
      res.status(err instanceof patternDiscovery.PatternDiscoveryError ? 400 : 500).json({ error: err.message });
    }
    return undefined;
  });

  /** The review queue — pending by default, or ?status=approved|rejected to see past decisions. */
  app.get('/api/pattern-engine/proposals', permissions.requireRole('admin'), (req, res) => {
    const status = typeof req.query.status === 'string' ? req.query.status : 'pending';
    res.json({ proposals: indexDb.listProposals(status) });
  });

  app.post('/api/pattern-engine/proposals/:id/approve', permissions.requireRole('admin'), (req, res) => {
    try {
      const proposal = patternDiscovery.approveProposal(LIBRARY, indexDb, req.params.id);
      res.json({ proposal });
    } catch (err) {
      res.status(err instanceof patternDiscovery.PatternDiscoveryError ? 400 : 500).json({ error: err.message });
    }
  ...
  app.post('/api/pattern-engine/proposals/:id/reject', permissions.requireRole('admin'), (req, res) => {
    try {
      const proposal = patternDiscovery.rejectProposal(indexDb, req.params.id);
      res.json({ proposal });
    } catch (err) {
      res.status(err instanceof patternDiscovery.PatternDiscoveryError ? 400 : 500).json({ error: err.message });
    }
  ...
  /** Undo an approved proposal — reverses its rule (if any) and demotes that action type's trust straight back to 'ask'. */
  app.post('/api/pattern-engine/proposals/:id/revert', permissions.requireRole('admin'), (req, res) => {
    try {
      const proposal = patternDiscovery.revertProposal(LIBRARY, indexDb, req.params.id);
      Object.assign(config, configLib.load()); // trust.js just wrote to config.json directly; keep this process's copy in sync
      res.json({ proposal });
    } catch (err) {
      res.status(err instanceof patternDiscovery.PatternDiscoveryError ? 400 : 500).json({ error: err.message });
    }
  ...
  // --- Phase O2: graduated trust, per action type -----------------------------

  /** Every known action type's current trust level and whether it has earned an offer to be promoted. */
  app.get('/api/trust', permissions.requireRole('admin'), (req, res) => {
    const actionTypes = trustLib.KNOWN_ACTION_TYPES.map((actionType) => ({
      actionType,
      level: trustLib.getTrustLevel(config, actionType),
      ...trustLib.promotionEligibility(indexDb, config, actionType),
    }));
    res.json({ actionTypes });
  });

  app.post('/api/trust/:actionType', permissions.requireRole('admin'), (req, res) => {
    const { level } = req.body || {};
    try {
      trustLib.setTrustLevel(req.params.actionType, level);
      Object.assign(config, configLib.load()); // keep this process's in-memory config in sync with what was just written
      res.json({ actionType: req.params.actionType, level: trustLib.getTrustLevel(config, req.params.actionType) });
    } catch (err) {
      res.status(err instanceof trustLib.TrustError ? 400 : 500).json({ error: err.message });
    }
  });

  /** What Ghost Mode's trust ladder has done on its own — never a direct human action. */
  app.get('/api/audit-log', permissions.requireRole('admin'), (req, res) => {
    const actionType = typeof req.query.actionType === 'string' ? req.query.actionType : null;
    res.json({ entries: indexDb.listAuditLog({ actionType }) });
  });

  // --- Phase O3: the assistant chat loop ---------------------------------

  app.get('/api/assistant/models', permissions.requireRole('admin'), async (req, res) => {
    const host = config.assistant?.host || assistant.DEFAULT_HOST;
    const installed = await nlRules.listModels({ host });
    const selected = config.assistant?.model || assistant.DEFAULT_MODEL;
    res.json({
      host,
      installed,
      selected,
      reachable: installed.length > 0,
      selectedInstalled: installed.includes(selected),
      // The cloud key itself is never returned by any route — only whether
      // one is currently set up, and whether this machine even has
      // somewhere secure to keep it (the OS keychain, desktop-app-only —
      // the same gate lib/central-index.js's passphrase already has).
      cloudAvailable: secretsStore.available,
      cloudConfigured: Boolean(config.assistant?.cloudKey),
      cloudModel: config.assistant?.cloudModel || assistant.DEFAULT_CLOUD_MODEL,
      cloudDisclosure: assistant.CLOUD_DISCLOSURE,
    });
  });

  app.post('/api/assistant/model', permissions.requireRole('admin'), (req, res) => {
    const { model } = req.body || {};
    if (typeof model !== 'string' || !model.trim()) return res.status(400).json({ error: 'Pick a model' });
    config.assistant = { ...(config.assistant || {}), model: model.trim() };
    configLib.save(config);
    res.json({ ok: true, selected: config.assistant.model });
  });

  /** Store (or replace) the cloud API key, encrypted at rest — never logged, never returned by any route afterward. */
  app.post('/api/assistant/cloud-key', permissions.requireRole('admin'), (req, res) => {
    const { apiKey, model } = req.body || {};
    if (typeof apiKey !== 'string' || !apiKey.trim()) return res.status(400).json({ error: 'Enter an API key' });
    if (!secretsStore.available) {
      return res.status(400).json({
        error: 'This machine has no secure place to remember the key (no OS keychain available), '
          + 'so the cloud backend cannot be set up here.',
      });
    }
    config.assistant = {
      ...(config.assistant || {}),
      cloudKey: secretsStore.encrypt(apiKey.trim()),
      cloudModel: (model && model.trim()) || config.assistant?.cloudModel || assistant.DEFAULT_CLOUD_MODEL,
    };
    configLib.save(config);
    res.json({ ok: true });
    return undefined;
  ...
  app.delete('/api/assistant/cloud-key', permissions.requireRole('admin'), (req, res) => {
    if (config.assistant) {
  ...
  /**
   * One conversation turn. `conversation` is whatever converse() returned
   * as `messages` last time — the client holds it, this stays stateless
   * server-side, the same shape a plain chat completion API already has.
   * Omitting it (or sending []) starts a fresh conversation, which is also
   * the only time memory examples are looked up — mid-conversation, the
   * system prompt they were folded into has already been sent.
   *
   * `backend: 'cloud'` is a deliberate, per-message choice the caller makes
   * — never a default this route falls back to on its own.
   */
  app.post('/api/assistant/message', permissions.requireRole('admin'), async (req, res) => {
    const { message, conversation, backend } = req.body || {};
    if (typeof message !== 'string' || !message.trim()) return res.status(400).json({ error: 'Say something first' });
    const useCloud = backend === 'cloud';
    if (useCloud && (!secretsStore.available || !config.assistant?.cloudKey)) {
      return res.status(400).json({ error: 'No cloud API key is set up — add one first.' });
    }
    try {
      const priorConversation = Array.isArray(conversation) ? conversation : [];
      const memoryExamples = priorConversation.length ? [] : assistantMemory.closestExamples(indexDb, message);
      ...
      const result = await assistant.converse({
        ...
        host: config.assistant?.host || assistant.DEFAULT_HOST,
        model: config.assistant?.model || assistant.DEFAULT_MODEL,
        apiKey: useCloud ? secretsStore.decrypt(config.assistant.cloudKey) : null,
        cloudModel: config.assistant?.cloudModel || assistant.DEFAULT_CLOUD_MODEL,
        memoryExamples,
      });

      // A save_rule call that actually executed (not just previewed) is
      // accepted-rule memory made real — see lib/assistant-memory.js.
      const savedCall = result.toolLog.find((t) => t.name === 'save_rule' && t.ranFor === 'real' && t.result?.saved);
      if (savedCall?.args?.text) {
        indexDb.rememberInstruction({ instruction: message, ruleText: savedCall.args.text });
      }

      res.json(result);
    } catch (err) {
      res.status(err instanceof assistant.AssistantError ? 400 : 500).json({ error: err.message });
    }
    return undefined;
  });
  ...
  app.patch('/api/accounts/:username', permissions.requireRole('admin'), (req, res) => {
    try {
      ...
      if (req.body?.disabled === true) {
        ...
        // A password change is the one patch that *must* revoke. Changing a
        // password is what someone does when they believe it is compromised
        // — a stolen cookie is signed with the server secret, not the
        // password, so it would otherwise keep working for the full
        // sessionDays regardless of the new password. Anything else makes
        // "change your password" quietly useless as a response to a stolen
        // device or a leaked session.
        //
        // req.sessionId is only ever preserved when an admin changes their
        // *own* password: it belongs to this caller, so it matches one of
        // the target user's sessions in exactly that case and no other,
        // which keeps them signed in on the tab they are using while
        // signing out every other device. Changing someone else's password
        // matches nothing here and revokes all of theirs, as it should.
        sessions.revokeAllForUser(req.params.username, req.sessionId);
      }
  ...
  return {
    ...
    syncWatcher: watcher,
    discovery: finder,
    // Exposed for the same reason syncWatcher and discovery are: something
    // outside this module (the desktop app's capture-device watcher, Phase
    // K) needs to ask the library's own index a question — here, "has this
    // file already been imported" — without re-opening a second handle to
...
async function start(config, options = {}) {
  const { app, LIBRARY, stopBackgroundTasks, syncWatcher: watcher, discovery: finder } = createApp(config, options);
