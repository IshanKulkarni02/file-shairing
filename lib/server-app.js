...
function createApp(config, { secrets = null } = {}) {
  ...
  app.get('/api/search/content', async (req, res) => {
    ...
    try {
      ...
      // Same over-fetch-then-filter reasoning as /api/search above: a
      // restricted account can lose most of a page to visibleRow() below.
      const rows = indexDb.searchByContent(queryVector, {
        model: clipLib.MODEL_ID,
        limit: restricted ? Math.min(limit * 5, 500) : limit,
      });
      ...
      for (const row of rows) {
        ...
        if (!visibleRow(req, row)) continue;
        results.push(indexDbLib.dbRowToResult(row));
      }
