...
function createApp(config, { secrets = null } = {}) {
  ...
  app.get('/api/search', async (req, res) => {
    ...
    // local account making this request. Running it for anyone below admin
    // would let a restricted local viewer transitively exercise a stored
    // admin-on-another-machine password it was never given. Skipped, not
    // refused: an ordinary local search must keep working for every role,
    // just without reaching past this machine — which is also why this
    // still goes through federatedSearch with no connections rather than a
    // separate code path: the response stays exactly one shape regardless
    // of who is asking or how far the search actually reached.
    const { results: merged, peers } = await federation.federatedSearch({
      library: LIBRARY,
      config: req.account.role === 'admin' ? config : { ...config, connections: [] },
      secrets: secretsStore,
      localResults,
      params,
    });

    res.json({ results: merged.slice(0, limit), peers, indexing: Boolean(indexScan) });
    return undefined;
  });
